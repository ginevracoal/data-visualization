...
ggplot(data,aes(x=id,y=1,fill=who))+
    geom_col()
...
```{r}
data$datew<-format(as.Date(data$date), "%Y-%m")
...
ww<-tokens_without_stop_words %>% group_by(who,datew,word) %>% summarise(count=n())
# average usage of word
wa<-ww %>% group_by(who,word) %>% summarise(avg=mean(count)) %>% arrange(-avg)
return(wa)
}
w<-get_words("trump")
